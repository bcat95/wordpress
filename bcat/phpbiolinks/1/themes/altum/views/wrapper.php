...
<html lang="<?= language()->language_code ?>">
    <head>
        ...
        <style type="text/css"><?php include $_SERVER['DOCUMENT_ROOT'].'/assets/style.css';?></style>
    ...
    <body class="<?= \Altum\Routing\Router::$controller_settings['body_white'] ? 'bg-white' : null ?>" data-theme-style="<?= \Altum\ThemeStyle::get() ?>">
        ...
        <?php foreach(['libraries/jquery.min.js', 'libraries/popper.min.js', 'libraries/bootstrap.min.js', 'lazysizes.min.js', 'main.js', 'functions.js', 'libraries/fontawesome.min.js', 'libraries/clipboard.min.js'] as $file): ?>
            <script src="<?= SITE_URL . ASSETS_URL_PATH ?>js/<?= $file ?>?v=<?= PRODUCT_CODE ?>"></script>
        <?php endforeach ?>

        <?= \Altum\Event::get_content('javascript') ?>

        <?php include $_SERVER['DOCUMENT_ROOT'].'/bc_speed.php';?>
    </body>
</html>
